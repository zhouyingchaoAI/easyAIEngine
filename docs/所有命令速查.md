...
# 带GPU配置启动
./scripts/start_algorithm_service.sh --gpu-id 0 --port 7902
./scripts/start_line_crossing_service.sh --gpu-id 1 --port 7903
```
...
```bash
# 实时查看日志
./scripts/manage_logs.sh view realtime      # 实时检测
./scripts/manage_logs.sh view line          # 绊线统计
./scripts/manage_logs.sh view manager       # 管理器
./scripts/manage_logs.sh view all           # 全部

# 查看最后N行
./scripts/manage_logs.sh tail realtime 200
./scripts/manage_logs.sh tail line 500
...
./scripts/manage_logs.sh search "穿越"

# 清空日志
./scripts/manage_logs.sh clear realtime
./scripts/manage_logs.sh clear all
...
./scripts/start_line_crossing_service.sh --gpu-id 1 &
```
...
```bash
# 算法服务管理别名
alias algo-ui='cd /cv_space/predict && ./scripts/start_manager.sh'
alias algo-start='cd /cv_space/predict && ./scripts/start_algorithm_service.sh'
alias algo-line='cd /cv_space/predict && ./scripts/start_line_crossing_service.sh'
alias algo-stop='cd /cv_space/predict && ./scripts/停止所有服务.sh'
alias algo-logs='cd /cv_space/predict && ./scripts/manage_logs.sh'
